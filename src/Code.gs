...
function validateFormData(formObject) {
  ...
  for (const field of requiredFields) {
    ...
    if (!value || typeof value !== "string" || value.trim().length === 0) {
      ...
      if (
        FIELD_LIMITS[field] &&
        sanitizedData[field].length > FIELD_LIMITS[field]
      ) {
        errors.push(
          `${field} exceeds maximum length of ${FIELD_LIMITS[field]} characters`
        );
      }
    }
  }
...
function getStudentNamesOptimized() {
  try {
    ...
    const data = sheet
      .getRange(2, dropDownColIndex + 1, lastRow - 1, 1)
      .getValues();
    const names = data
      .map((row) => row[0])
...
function getSubmissionDataOptimized(submissionId) {
  try {
    ...
    throw new Error(
      `Submission ID '${submissionId}' was not found in any sheet.`
    );
  } catch (e) {
...
function processFormSecure(formObject) {
  try {
    ...
    Logger.log(
      "PROCESS FORM DEBUG - timeOfIncident in formObject:",
      formObject.timeOfIncident
    );
    Logger.log(
      "PROCESS FORM DEBUG - timeOfIncident in cleanData:",
      cleanData.timeOfIncident
    );
    Logger.log(
      "PROCESS FORM DEBUG - typeof timeOfIncident:",
      typeof cleanData.timeOfIncident
    );
    ...
    if (cleanData.MinorInfractions) {
      infractions = Array.isArray(cleanData.MinorInfractions)
        ? cleanData.MinorInfractions.join(", ")
        : cleanData.MinorInfractions;
    } else if (cleanData.MajorInfractions) {
      infractions = Array.isArray(cleanData.MajorInfractions)
        ? cleanData.MajorInfractions.join(", ")
        : cleanData.MajorInfractions;
    }

    if (cleanData.infractionOtherText && cleanData.infractionOtherText.trim()) {
      infractions = infractions.replace(
        /\bOther\b/,
        `Other: ${sanitizeInput(cleanData.infractionOtherText)}`
      );
    }
    ...
    if (cleanData.MinorActionsTaken) {
      actionsTaken = Array.isArray(cleanData.MinorActionsTaken)
        ? cleanData.MinorActionsTaken.join(", ")
        : cleanData.MinorActionsTaken;
    } else if (cleanData.MajorActionsTaken) {
      actionsTaken = Array.isArray(cleanData.MajorActionsTaken)
        ? cleanData.MajorActionsTaken.join(", ")
        : cleanData.MajorActionsTaken;
    }

    if (
      cleanData.actionsTakenOtherText &&
      cleanData.actionsTakenOtherText.trim()
    ) {
      actionsTaken = actionsTaken.replace(
        /\bOther\b/,
        `Other: ${sanitizeInput(cleanData.actionsTakenOtherText)}`
      );
    }
    ...
    const newRow = headers.map((header) => {
      Logger.log(
        "DEBUG - Right before switch, cleanData has: " +
          JSON.stringify(cleanData)
      );
      Logger.log(
        "DEBUG - cleanData.timeOfIncident specifically: " +
          (cleanData.timeOfIncident || "UNDEFINED")
      );
      ...
      switch (header) {
        ...
        case "GradeLevel":
          return studentDetails.gradeLevel;
        case "Team":
          return studentDetails.team;
        case "StaffName":
          return cleanData.staffName;
        case "StaffEmail":
          return cleanData.staffEmail;
        case "DateOfIncident":
          return `'${formatDateDisplay(cleanData.dateOfIncident)}`;
        case "TimeOfIncident":
          let timeValue =
            cleanData.timeOfIncident ||
            cleanData.TimeOfIncident ||
        ...
        case "DescriptionOfInfraction":
          return cleanData.descriptionOfInfraction;
        ...
        case "DetailsOfCommunication":
          return cleanData.DetailsOfCommunication || "";
        case "Comments":
          return cleanData.Comments || "";
        // FIXED: Process perceived motivation properly
        case "PerceivedMotivation":
          return Array.isArray(cleanData.PerceivedMotivation)
            ? cleanData.PerceivedMotivation.join(", ")
            : cleanData.PerceivedMotivation || "";
        case "Infraction":
          return infractions;
        case "ActionsTaken":
          return actionsTaken;
        case "SubmissionNumber":
          return submissionId;
        case "Timestamp":
          return `'${formatDateDisplay(new Date())} at ${formatTimeDisplay(
            new Date()
          )}`;
        case "Status":
          return "Pending Admin Review";
        case "DayOfWeek":
          return new Date(cleanData.dateOfIncident).toLocaleString("en-US", {
            weekday: "long",
          });
        case "SubmittedBy":
          return userEmail;
    ...
    Logger.log(
      `Successful submission: ${submissionId} by ${userEmail} for student ${studentDetails.studentName}`
    );
...
function sendNotificationEmail(
  ...
  try {
    ...
    const infractions =
      formData.MinorInfractions || formData.MajorInfractions || "Not specified";
...
function createEmailDraftWithPDF(referralData) {
  try {
    ...
    const submissionResponse = getSubmissionDataOptimized(
      referralData.submissionNumber
    );
    ...
    const tempDoc = DocumentApp.create(
      `TEMP_${referralData.submissionNumber}_${Date.now()}`
    );
    ...
    body
      .appendParagraph(`Student: ${referralData.studentName}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Submission ID: ${referralData.submissionNumber}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Date of Incident: ${referralData.dateOfIncident}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Location: ${referralData.location}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Staff Member: ${referralData.staffName}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Type: ${referralData.referralType} Referral`)
      .setAttributes(normalStyle);
    ...
    body
      .appendParagraph(
        referralData.infraction ||
          fullSubmissionData.Infraction ||
          "Not specified"
      )
      .setAttributes(normalStyle);
    ...
    body
      .appendParagraph(
        referralData.description ||
          fullSubmissionData.DescriptionOfInfraction ||
          "Not provided"
      )
      .setAttributes(normalStyle);
    ...
    if (fullSubmissionData.AdminConsequence) {
      body.appendParagraph("").setSpacingAfter(10);
      const adminActionsLabel = body.appendParagraph(
        "ADMINISTRATIVE CONSEQUENCES:"
      );
      adminActionsLabel.setAttributes(labelStyle);
      body
        .appendParagraph(fullSubmissionData.AdminConsequence)
        .setAttributes(normalStyle);
    }

    if (referralData.adminComments || fullSubmissionData.AdminComments) {
      body.appendParagraph("").setSpacingAfter(10);
      const adminCommentsLabel = body.appendParagraph(
        "ADMINISTRATIVE COMMENTS:"
      );
      adminCommentsLabel.setAttributes(labelStyle);
      body
        .appendParagraph(
          referralData.adminComments || fullSubmissionData.AdminComments
        )
        .setAttributes(normalStyle);
    }

    if (
      fullSubmissionData.RewardTripPts &&
      fullSubmissionData.RewardTripPts !== ""
    ) {
      body.appendParagraph("").setSpacingAfter(10);
      ...
      body
        .appendParagraph(fullSubmissionData.RewardTripPts + " points")
        .setAttributes(normalStyle);
    }
    ...
    if (fullSubmissionData.DCPoints && fullSubmissionData.DCPoints !== "") {
      ...
      body
        .appendParagraph(fullSubmissionData.DCPoints + " points")
        .setAttributes(normalStyle);
    }
    ...
    body
      .appendParagraph(
        `Generated: ${formatDateDisplay(new Date())} at ${formatTimeDisplay(
          new Date()
        )}`
      )
      .setAttributes(normalStyle);
...
${userEmail}
    ...
    try {
      ...
      Logger.log(
        "Gmail draft created with attachment in administrator account"
      );
    } catch (gmailError) {
      Logger.log("Gmail draft creation error: " + gmailError.message);
      // Fallback: Try without the 'from' parameter
      const draft = GmailApp.createDraft("", subject, body_text, {
        attachments: [pdfBlob],
      });
    ...
    Logger.log(
      `Email draft created with PDF for ${referralData.submissionNumber}`
    );
...
function createGoogleDocExport(formData, submissionData) {
  try {
    ...
    body
      .appendParagraph(`Student: ${submissionData.StudentName}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Submission ID: ${submissionData.SubmissionNumber}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(
        `Date of Incident: ${submissionData.DateOfIncident || "Not specified"}`
      )
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Location: ${submissionData.Location}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Staff Member: ${submissionData.StaffName}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(
        `Type: ${formData.sheetName || "Not specified"} Referral`
      )
      .setAttributes(normalStyle);
    ...
    body
      .appendParagraph(`Name: ${submissionData.StudentName}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`ID: ${submissionData.StudentID}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Grade: ${submissionData.GradeLevel}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Team: ${submissionData.Team}`)
      .setAttributes(normalStyle);
    ...
    body
      .appendParagraph(
        `Time of Incident: ${submissionData.TimeOfIncident || "Not specified"}`
      )
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Day of Week: ${submissionData.DayOfWeek}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(`Location: ${submissionData.Location}`)
      .setAttributes(normalStyle);
    body
      .appendParagraph(
        `Referring Staff: ${submissionData.StaffName} (${submissionData.StaffEmail})`
      )
      .setAttributes(normalStyle);
    ...
    body
      .appendParagraph(
        `Infractions: ${submissionData.Infraction || "Not specified"}`
      )
      .setAttributes(normalStyle);

    const descriptionLabel = body.appendParagraph("Description:");
    descriptionLabel.setAttributes(labelStyle);
    const descriptionText = body.appendParagraph(
      submissionData.DescriptionOfInfraction || "Not provided"
    );
    ...
    body
      .appendParagraph(
        `Previous Actions Taken: ${
          submissionData.ActionsTaken || "None specified"
        }`
      )
      .setAttributes(normalStyle);
    body
      .appendParagraph(
        `Perceived Motivation: ${
          submissionData.PerceivedMotivation || "Not specified"
        }`
      )
      .setAttributes(normalStyle);
    ...
    body
      .appendParagraph(
        `Home Communication: ${submissionData.HomeCommunications || "No"}`
      )
      .setAttributes(normalStyle);
    if (submissionData.DetailsOfCommunication) {
      body
        .appendParagraph(
          `Communication Details: ${submissionData.DetailsOfCommunication}`
        )
        .setAttributes(normalStyle);
    }
    ...
    if (formData.ParentContactDate || formData.ParentContactMethod) {
      body
        .appendParagraph(
          `Admin Parent Contact Date: ${
            formData.ParentContactDate || "Not specified"
          }`
        )
        .setAttributes(normalStyle);
      body
        .appendParagraph(
          `Admin Contact Method: ${
            formData.ParentContactMethod || "Not specified"
          }`
        )
        .setAttributes(normalStyle);
      if (formData.ParentContactNotes) {
        body
          .appendParagraph(
            `Admin Contact Notes: ${formData.ParentContactNotes}`
          )
          .setAttributes(normalStyle);
      }
    }
    ...
    if (formData.CodeOfConduct) {
      body
        .appendParagraph(
          `Code of Conduct Violations: ${formData.CodeOfConduct}`
        )
        .setAttributes(normalStyle);
    }
    ...
    if (formData.CRDCReporting) {
      body
        .appendParagraph(
          `Ohio State Reporting Codes: ${formData.CRDCReporting}`
        )
        .setAttributes(normalStyle);
    }
    ...
    if (formData.AdminConsequence) {
      body
        .appendParagraph(
          `Administrative Consequences: ${formData.AdminConsequence}`
        )
        .setAttributes(normalStyle);
    }
    ...
    if (formData.RewardTripPts !== undefined && formData.RewardTripPts !== "") {
      body
        .appendParagraph(
          `Reward Trip Points Deducted: ${formData.RewardTripPts}`
        )
        .setAttributes(normalStyle);
    }
    ...
    if (formData.DCPoints !== undefined && formData.DCPoints !== "") {
      body
        .appendParagraph(`DC Trip Points Deducted: ${formData.DCPoints}`)
        .setAttributes(normalStyle);
    }
    ...
    const footer = body.appendParagraph(
      "--- End of Discipline Referral Report ---"
    );
...
function updateRecordSecure(formObject) {
  try {
    ...
    const textFinder = sheet
      .createTextFinder(formObject.submissionNumber)
      .matchEntireCell(true);
    const foundRange = textFinder.findNext();

    if (!foundRange) {
      throw new Error(
        `Submission ID ${formObject.submissionNumber} not found.`
      );
    }
    ...
    Object.keys(formObject).forEach((key) => {
      if (typeof formObject[key] === "string") {
        ...
        if (
          FIELD_LIMITS[key] &&
          sanitizedFormObject[key].length > FIELD_LIMITS[key]
        ) {
          throw new Error(
            `${key} exceeds maximum length of ${FIELD_LIMITS[key]} characters`
          );
        }
      } else {
    ...
    if (
      sanitizedFormObject.adminConsequenceOtherText &&
      sanitizedFormObject.adminConsequenceOtherText.trim()
    ) {
      sanitizedFormObject.AdminConsequence = (
        sanitizedFormObject.AdminConsequence || ""
      ).replace(
        "Other",
        `Other: ${sanitizedFormObject.adminConsequenceOtherText}`
      );
    }

    const updatedBy = userEmail;
    const updatedAt =
      formatDateDisplay(new Date()) + " at " + formatTimeDisplay(new Date());
    ...
    Logger.log(
      `Successful update: ${sanitizedFormObject.submissionNumber} by ${userEmail}`
    );
...
Completed at: ${formatDateDisplay(new Date())} at ${formatTimeDisplay(
      new Date()
    )}
...
This referral is now closed.
    ...
    if (
      staffEmail &&
      staffEmail.trim() &&
      staffEmail !== SETTINGS.FINAL_EMAIL
    ) {
...
function autoSaveRecord(formObject) {
  try {
    ...
    Logger.log(
      `Auto-saved draft for ${formObject.submissionNumber} by ${userEmail}`
    );
...
function processPositiveReferral(formObject, userEmail) {
  try {
    const studentDetails = getStudentInfoByNameOptimized(
    ...
    const newRow = headers.map((header) => {
      switch (header) {
        ...
        case "Team":
          return studentDetails.team;
        case "StaffName":
          return sanitizeInput(formObject.staffName);
        case "StaffEmail":
          return sanitizeInput(formObject.staffEmail);
        case "DateOfIncident":
          return `'${formatDateDisplay(formObject.dateOfIncident)}`;
        case "PositiveBehavior":
          return positiveBehavior;
        case "PositiveDetails":
          return positiveDetails;
        case "Comments":
          return sanitizeInput(formObject.Comments || "");
        case "SubmissionNumber":
          return submissionId;
        case "Timestamp":
          return `'${formatDateDisplay(new Date())} at ${formatTimeDisplay(
    ...
    Logger.log(
      `Successful positive submission: ${submissionId} by ${userEmail} for student ${studentDetails.studentName}`
    );
...
function sendPositiveNotificationEmail(
  submissionId,
  studentDetails,
  formData,
  submitterEmail
